...
number_snps<-200
overlapping_windows<-100
vcf_file_name<-"both_ed_I.vcf.recode.vcf"
...
#                       pop_JB22_nomixed_samples=paste(pop_JB22_nomixed, collapse = '_'))
  ...
  if (((ccm_pca$eigenvect[ccm_pca$sample==paste0("JB22_",chromosome,"_ILL"),1]-min(PC1_component))/(max(PC1_component)-min(PC1_component)))<0.4){
    Nor_PC1_pol<-(((PC1_component-min(PC1_component))/(max(PC1_component)-min(PC1_component))))
  } else if (((ccm_pca$eigenvect[ccm_pca$sample==paste0("JB22_",chromosome,"_ILL"),1]-min(PC1_component))/(max(PC1_component)-min(PC1_component)))>0.6) {
